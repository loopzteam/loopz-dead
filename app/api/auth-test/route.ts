...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    logs.push(
      `RLS test successful! Found ${loopzData?.length || 0} loopz records accessible to this user`,
    );
    ...
    try {
      ...
      logs.push(
        `Insert exception: ${insertException instanceof Error ? insertException.message : String(insertException)}`,
      );
    }
