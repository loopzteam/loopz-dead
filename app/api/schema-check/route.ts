...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    try {
      const { data: loopzInfo, error: loopzError } = await supabase.rpc(
        'get_table_columns',
        { table_name: 'loopz' }
      );
      ...
      if (loopzError) {
        ...
        const { data: directInfo, error: directError } = await supabase.from('information_schema.columns')
          .select('column_name')
    ...
    try {
      const { data, error } = await supabase.from('loopz')
        .insert({
          title: 'Schema Test',
