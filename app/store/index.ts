...
type StoreState = AuthSlice & LoopzSlice & TaskSlice & ChatSlice & UISlice;
...
export const useStore = create<StoreState>((set, get) => ({
  ...
  updateLoopz: (updatedLoopz) => set((state) => ({
...
export const loadUserData = async () => {
  ...
  try {
    ...
    store.setLoopzList(loopzList || []);
    
  } catch (error) {
    console.error('Error loading user data:', error);
    store.setLoopzError(error instanceof Error ? error.message : 'Failed to load data');
  } finally {
    store.setAuthLoading(false);
    store.setLoopzLoading(false);
  }
};

export const loadLoopzTasks = async (loopzId: string) => {
  const store = useStore.getState();
  ...
  try {
    ...
    store.setTasks(tasksWithUIState);
    ...
    if (tasksWithUIState.length > 0) {
      const { data: microsteps, error: microstepsError } = await supabase
        .from('microsteps')
        .select('*')
        .in('task_id', tasksWithUIState.map(t => t.id))
        .order('position', { ascending: true });
        
      if (microstepsError) throw microstepsError;
      store.setMicrosteps(microsteps || []);
    }
  } catch (error) {
    console.error('Error loading loop tasks:', error);
    store.setTaskError(error instanceof Error ? error.message : 'Failed to load tasks');
  } finally {
