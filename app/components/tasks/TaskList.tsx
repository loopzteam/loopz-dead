...
export default function TaskList({ loopzId }: TaskListProps) {
  ...
  const { 
    ...
    // Make sure tasks exists and is an array before filtering
    ...
    return {
      ...
      taskLoading: state.taskLoading || false,
      taskError: state.taskError || null,
      loadLoopzTasks: () => {} // We'll need to add this to the store later
    };
  });
  ...
  // Get microsteps for a task
  const getTaskMicrosteps = (taskId: string): Microstep[] => {
    return microsteps.filter(ms => ms.task_id === taskId).sort((a, b) => a.position - b.position);
  };

  // Handle task click - selects the task
  const handleTaskClick = (taskId: string) => {
    setSelectedTaskId(selectedTaskId === taskId ? null : taskId);
  };

  // Handle expand click - shows/hides microsteps
  const handleExpandClick = (taskId: string, e: React.MouseEvent) => {
  ...
  const handleToggleMicrostep = (microstepId: string) => {
  ...
  useEffect(() => {
    if (loopzId) {
      // This will be implemented when we add the function to the store
      // loadLoopzTasks(loopzId);
    }
  }, [loopzId]);
                    ...
                    <li key={microstep.id} className="flex items-center">
                      <div 
                        className={`mr-3 w-4 h-4 rounded-full border flex items-center justify-center ${
                          microstep.is_completed ? 'bg-black border-black text-white' : 'border-gray-300'
                        }`}
