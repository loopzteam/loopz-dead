...
const openai = isClient ? new OpenAI({
...
export async function generateClarification(messages: Message[]): Promise<string> {
  try {
    ...
    return data.clarification || 'I understand your situation. Let me help you organize your thoughts.';
  } catch (error) {
...
export async function generateTasksFromThread(messages: Message[]): Promise<string[]> {
  try {
    ...
    const data = await response.json();
    return data.tasks || ["Review your situation", "Identify key priorities", "Create a simple plan"];
  } catch (error) {
 ...
 */
export async function generateMicrosteps(taskId: string, taskTitle: string, loopContext: string): Promise<string[]> {
  try {
