...
async function setupDatabase() {
  ...
  try {
    ...
    try {
      ...
      await supabase.schema.createTable('loops', {
        id: { type: 'uuid', primaryKey: true, default: supabase.sql`gen_random_uuid()` },
        title: { type: 'text', notNull: true },
        user_id: { type: 'uuid', notNull: true, references: 'auth.users.id', onDelete: 'cascade' },
        created_at: { type: 'timestamptz', notNull: true, default: supabase.sql`now()` },
        updated_at: { type: 'timestamptz', notNull: true, default: supabase.sql`now()` }
      }).ifNotExists();
      
      // 2. Create tasks table
      console.log('Creating tasks table...');
      await supabase.schema.createTable('tasks', {
        id: { type: 'uuid', primaryKey: true, default: supabase.sql`gen_random_uuid()` },
        loop_id: { type: 'uuid', notNull: true, references: 'loops.id', onDelete: 'cascade' },
